...
App 已内置服务器地址，用户装完即可用，**不需要手动填**。

地址定义在客户端一处常量：

```ts
// apps/mobile/src/network/serverConfig.ts
export const DEFAULT_SERVER = 'http://47.239.14.144:8787';
```

**换地址时只改这一行**（例如日后上了域名、改为 HTTPS：
`export const DEFAULT_SERVER = 'https://你的域名';`），重新构建即可。

> 早期版本在登录页暴露了"修改服务器"输入框，已移除 ——
> 那是调试期产物，普通用户不该看到，也不该被误改。
...
服务端曾出现"每隔一段时间突然挂掉、且 `pm2 logs` 没有任何错误"的情况。

原因是**内存只增不减**：过期令牌只在"被访问时"才删除，客户端一旦不再访问
（换设备、卸载），那条记录就永远留在内存里，还会被写进快照。
涨到一定程度被系统 OOM 杀掉 —— 系统杀进程不留 JS 堆栈，所以日志是空的。

现在服务端启动时会开启定期维护（每 30 分钟），回收三类数据：

| 数据 | 清理规则 |
|---|---|
| 过期令牌 | TTL 到期即回收（无需等被访问） |
| 陈旧在线状态 | 超过 90 天未活跃 |
| 已投递信封 | 已送达且超过 7 天（兜底，正常 ack 后就删了） |

同时每 30 分钟打一次内存概况到日志，形如：

```
[mem] users=12 devices=15 tokens=18 envelopes=3 opk=240 heap=64MB
```

OOM 时这条日志就是线索。查看：

```bash
pm2 logs e2ee-server --lines 100 | grep mem
```

**兜底仍建议保留** PM2 的内存重启阈值：

```bash
pm2 start npm --name e2ee-server --max-memory-restart 350M -- run start
```
