...
先看专门的排查指南：**[docs/DEBUG-WHITE-SCREEN.md](docs/DEBUG-WHITE-SCREEN.md)**，里面讲了怎么抓到"真正含崩溃"的日志——大部分白屏排查卡在这里，抓到的日志是 App 切到后台之后才开始记的，自然什么都没有。

App 现在会把未捕获异常落盘，下次启动在登录页直接显示（红色"上次启动异常"卡片，可展开堆栈），不用连电脑也能看到原因。
